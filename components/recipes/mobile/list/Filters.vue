...
const { data: categories, pending: pendingCategories } = useFetch<ICategory[]>('/api/categories', {
  key: 'categories',
  server: false,
  default: () => []
})
  ...
  <UDrawer>
    <div class="self-end fixed bottom-18">
      <UChip v-if="nbSelectedFilters" :text="nbSelectedFilters" size="3xl">
        <UButton color="primary" class="w-fit rounded-full shadow-lg" :loading icon="material-symbols:filter-list-rounded" size="xl" />
      </UChip>

      <UButton v-else color="primary" class="w-fit rounded-full shadow-lg" :loading icon="material-symbols:filter-list-rounded" size="xl" />
    </div>

    <template #body>
      <div class="space-y-4">
        <UFormField label="Nom" help="3 caractères minimum">
          <UInput v-model="filters.search" placeholder="Ex : Sushis" class="w-full">
            <template #trailing>
              <UButton
                color="neutral"
                variant="link"
    ...
    <template #footer>
      <UButton label="Réinitialiser" :loading variant="outline" color="neutral" block @click="handleResetFilter" />
    </template>
  </UDrawer>
</template>